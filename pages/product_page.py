...
class ProductPage(BasePage):
    def should_be_params_in_link(self):
        assert "?promo=" in str(self.browser.current_url),\
            "Promo code is wrong or not presented"
    ...
    def check_bookname(self):
        ...
        assert text_of_allert_product_name == text_of_product_name_on_page,\
            "Book name doesn't matcg"

    def check_price(self):
        text_allert_price = self.browser.find_element(
            *ProductPageLocators.ALLERT_PRICE).text
        text_page_price = self.browser.find_element(
            *ProductPageLocators.PAGE_PRICE).text
        assert text_allert_price == text_page_price,\
            "Price doesn't match"
